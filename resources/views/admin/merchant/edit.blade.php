...
@section('content')
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form method="post" action="/admin/merchant/{{ $merchant['id'] }}" enctype="multipart/form-data">
            @method('patch')
            @csrf
            <div class="card-body">
                <fieldset disabled>
                    <div class="form-group">
                        ...
                        <input class="form-control @error('manajer_id') is-invalid @enderror" type="text" id="manajer_id"
                            name="manajer_id" value="{{ $manajer->manajerHasMerchant->userManajer->nama_user ?? '' }}" />
                        @error('manajer_id')
                ...
                <div class=" form-group">
                    ...
                    <select class="custom-select form-control @error('sales_id') is-invalid @enderror" id="sales_id"
                        ...
                        @foreach ($sales as $value)
                            <option value="{{ $value->sales_id }}"
                                {{ $value->sales_id == $merchant->salesHasMerchant->sales_id ? 'selected' : '' }}>
                                {{ $value->userSales->nama_user ?? '' }}
                            </option>
                        @endforeach
                    </select>
                    @error('sales_id')
                ...
                <div class="forom-group mt-7">
                    ...
                    <a href="/admin/merchant/table" class="btn btn-secondary">Batal</a>
                </div>
            </div>
        </form>
        <!--end::Form-->
...
@section('JS')
    ...
    {{-- Script Dependent Dropdown SALES --}}
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery('select[name="manajer_id"]').on('change', function() {
                var manajerID = jQuery(this).val();
                if (manajerID) {
                    jQuery.ajax({
                        url: '/admin/merchant/editS/' + manajerID,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            console.log(data);
                            jQuery('select[name="sales_id"]').empty();
                            jQuery.each(data, function(key, value) {
                                $('select[name="sales_id"]').append('<option value="' +
                                    value.id + '">' + value.name + '</option>');
                            });
                        }
                    });
                } else {
                    $('select[name="sales_id"]').empty();
                }
            });
        });
    </script>
    ...
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery('select[name="province_id"]').on('change', function() {
                ...
                if (provinceID) {
                    jQuery.ajax({
                        url: '/admin/merchant/edit/' + provinceID,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            if (data) {
            ...
            jQuery('select[name="city_id"]').on('change', function() {
                ...
                if (cityID) {
                    jQuery.ajax({
                        url: '/admin/merchant/edit1/' + cityID,
                        type: "GET",
                        dataType: "json",
    ...
    <script>
        ...
        function initMap() {
            ...
            map.controls[
                google.maps.ControlPosition.TOP_CENTER].push(locationButton);

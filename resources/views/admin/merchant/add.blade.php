...
@section('content')
    ...
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form method="post" action="/admin/merchant/table" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class=" form-group">
                    <label>Nama Manajer *</label>
                    <select class="custom-select form-control dynamic @error('manajer_id') is-invalid @enderror"
                        id="manajer_id" name="manajer_id" data-dependent="sales_id" value="{{ old('manajer_id') }}">
                        <option value="">pilih manajer</option>
                        @foreach ($manajerdr as $key => $value)
                ...
                <div class=" form-group">
                    ...
                    <select class="custom-select form-control @error('sales_id') is-invalid @enderror" id="sales_id"
                        name="sales_id" value="{{ old('sales_id') }}">
                ...
                <div class=" form-group">
                    ...
                    <select class="custom-select form-control @error('kategori_id') is-invalid @enderror" id="kategori_id"
                        name="kategori_id" value="{{ old('nama_kategori') }}">
    ...
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery('select[name="manajer_id"]').on('change', function() {
                ...
                if (manajerID) {
                    jQuery.ajax({
                        url: '/admin/merchant/addS/' + manajerID,
                        type: "GET",
                        dataType: "json",
    ...
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery('select[name="province_id"]').on('change', function() {
                ...
                if (provinceID) {
                    jQuery.ajax({
                        url: '/admin/merchant/add/' + provinceID,
                        type: "GET",
                        dataType: "json",
                        success: function(data) {
                            if (data) {
            ...
            jQuery('select[name="city_id"]').on('change', function() {
                ...
                if (cityID) {
                    jQuery.ajax({
                        url: '/admin/merchant/add1/' + cityID,
                        type: "GET",
                        dataType: "json",
